...
def response(voice):
    ...
    if "sözlük " in voice:
        ...
        search = record("Hangi kelime")
        url = "https://context.reverso.net/çeviri/türkçe-ingilizce/{}".format(search)
        request=requests.get(url)

        html_icerigi=request.content

        soup=BeautifulSoup(html_icerigi,'html.parser')
        turkce=soup.find_all("div",{"class":"display-term"})

        for x in turkce:
            speak(x.text)
            print(x.text)

    if "eve geldim" in voice:
        speak('Hoş geldin kameraya yaklaş  ')
        print('Hoş geldin kameraya yaklaş  ')
        pyprog = 'yuztanima.py'
        call(['python', '-i', pyprog] )

    if "haberler nasıl" in voice:
        speak('Ekrana yansıtıyorum')
        url = 'https://www.ntv.com.tr/son-dakika'
        webbrowser.get().open(url)
        speak('Son dakika haberleri şu şekilde')
        print('Son dakika haberleri şu şekilde')
